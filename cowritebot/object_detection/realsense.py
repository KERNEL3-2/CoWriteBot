...
class ImgNode(Node):
    def __init__(self):
        ...
        self.color_subscription = self.create_subscription(
            Image, '/camera/realsense2/color/image_raw', self.color_callback, 10)
        self.depth_subscription = self.create_subscription(
            Image, '/camera/realsense2/depth/image_rect_raw', self.depth_callback, 10)
        self.camera_info_subscription = self.create_subscription(
            CameraInfo, '/camera/realsense2/color/camera_info', self.camera_info_callback, 10)
        self.get_logger().info("Img_node is initialized...")
